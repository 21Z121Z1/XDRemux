...
唯一可复用的 `research.yml` 工作流负责可移植研究测试和原生载体回读。
它按研究路径过滤，在 PR 或 main 的精确 SHA 上以只读权限运行，既不写入
仓库引用，也不提供产品 `exact-head` 检查。研究通过不代表假设晋升，
也不证明 Photos/Gallery 真机验收。实验命令与晋升门槛放在对应代码旁，
不要为每个实验新增一次性工作流。
