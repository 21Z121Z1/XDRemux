...
private enum LHDRExtractor {
    ...
    static func portraitPrivateTail(from data: Data) -> Data? {
        guard let manifestInfo = try? locateManifest(in: data) else { return nil }
        let names = Set(manifestInfo.entries.map(\.name))
        guard names.contains("src.image"), names.contains("rear.depth") else { return nil }
        for marker in qtiMarkers {
            guard let markerStart = firstIndex(of: marker, in: data) else { continue }
            let boxStart = markerStart - 4
            guard boxStart >= 0 else { continue }
            return data.subdata(in: boxStart..<data.count)
        }
        return nil
    }
...
struct LHDRToISOHDRCLI {
    ...
    Usage:
            ...
            XDRemux.swift convert --apple-portrait --input <oppo.heic> [--output <out.heic>]
    ...
    private static func runConvert(_ cmd: ConvertCommand) throws {
        ...
        let portraitTail: Data?
        if let sourceData = try? Data(contentsOf: cmd.inputURL, options: [.mappedIfSafe]) {
            portraitTail = LHDRExtractor.portraitPrivateTail(from: sourceData)
        } else {
            portraitTail = nil
        }
        ...
        if let portraitTail {
            let handle = try FileHandle(forWritingTo: cmd.outputURL)
            try handle.seekToEnd()
            try handle.write(contentsOf: portraitTail)
            try handle.close()
            print("preserved OPPO portrait private tail (\(portraitTail.count) bytes)")
        }
