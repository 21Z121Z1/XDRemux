...
fn apple_makernote_byte_order(note: &[u8]) -> Result<AppleMakerNoteByteOrder> {
    if !note.starts_with(APPLE_MAKERNOTE_PREFIX) || note.len() < 20 {
        return Err(invalid(
            "Texture Style source does not contain an Apple MakerNote",
        ));
    }
...
fn apple_makernote_entry(note: &[u8], wanted_tag: u16) -> Result<Option<AppleMakerNoteEntry>> {
    ...
    for index in 0..count {
        ...
        let unit = tiff_type_size(field_type).ok_or_else(|| {
            invalid(format!(
                "Apple MakerNote tag {wanted_tag} has unknown TIFF type {field_type}"
            ))
        })?;
...
fn build_minimal_texture_makernote(photo_identifier: &[u8], tag84: &[u8]) -> Result<Vec<u8>> {
    if photo_identifier.len() != 37 || photo_identifier.last() != Some(&0) || tag84.is_empty() {
        return Err(invalid(
            "Texture Style Apple MakerNote inputs are malformed",
        ));
    }
...
fn surgical_replace_texture_tag84(note: &[u8], tag84: &[u8]) -> Result<Vec<u8>> {
    ...
    if new_offset == output.len() {
        ...
        return Err(invalid(
            "Apple MakerNote tag84 replacement offset is inconsistent",
        ));
    }
...
fn validate_output(data: &[u8], expected_alias_count: usize) -> Result<()> {
    ...
    let photo = apple_makernote_entry(&note, APPLE_PHOTO_IDENTIFIER_TAG)?
...
mod tests {
    ...
    fn verified_texture_contract_constants_are_stable() {
        ...
        let note =
            build_minimal_texture_makernote(&photo, &texture_tag84_bplist().unwrap()).unwrap();
    ...
    fn apple_makernote_patch_preserves_photo_identifier_and_retargets_tag84() {
        ...
        let old_photo = apple_makernote_entry(&note, APPLE_PHOTO_IDENTIFIER_TAG)
            ...
            .unwrap();
        let old_photo_bytes =
            note[old_photo.value_offset..old_photo.value_offset + old_photo.value_length].to_vec();
