...
DATASET_SCHEMA = "xdremux-universal-photographic-style-dataset-v2"
...
def normalized_primary_rgb(styled: np.ndarray) -> np.ndarray:
    """Interpret integer RGB in [0, 255] or floating RGB in [0, 1], never by extrema."""
    source = np.asarray(styled)
    if source.shape != (3, INPUT_SIZE, INPUT_SIZE):
        raise ReverseKey1Error(f"styled primary must be 3x{INPUT_SIZE}x{INPUT_SIZE}")
    if source.dtype.kind not in "uif" or not np.isfinite(source).all():
        raise ReverseKey1Error("styled primary must contain finite real RGB values")
    maximum = 1.0 if source.dtype.kind == "f" else 255
    if np.any(source < 0) or np.any(source > maximum):
        raise ReverseKey1Error("integer RGB must be in [0, 255]; floating RGB must be in [0, 1]")
    value = source.astype(np.float32)
    return value if source.dtype.kind == "f" else value / 255.0
...
def primary_image_features(styled: np.ndarray) -> np.ndarray:
    ...
    value = normalized_primary_rgb(styled)
...
def _sidecar_array(path: Path, preferred_key: str, expected_sha: str | None = None) -> np.ndarray:
    payload = path.read_bytes()
    if expected_sha is not None and hashlib.sha256(payload).hexdigest() != expected_sha:
        raise ReverseKey1Error(f"optional modality identity mismatch: {path}")
...
def linear_sidecar_features(path: Path, *, expected_sha: str | None = None) -> np.ndarray:
    value = np.asarray(_sidecar_array(path, "linear_rgb", expected_sha), dtype=np.float32)
...
def gain_map_sidecar_features(path: Path, *, expected_sha: str | None = None) -> np.ndarray:
    return gain_map_features(_sidecar_array(path, "gain_map", expected_sha))
...
def _load_optional_modalities_manifest(
    ...
    for item in samples:
        ...
        if re.fullmatch(r"[0-9a-f]{64}", source_sha) is None or source_sha in by_sha:
            raise ReverseKey1Error(
        ...
        for field, feature_builder in (
            ...
            feature_builder(sidecar, expected_sha=sidecar_sha)
            prepared[field] = str(sidecar)
            prepared[field.removesuffix("Path") + "SHA256"] = sidecar_sha
        if len(prepared) == 1:
            raise ReverseKey1Error(
...
def metadata_vector(
    ...
    *,
    ...
    input_bit_depth = _number(input_bit_depth)
    ...
    for index, value in enumerate(raw_values):
        if value is not None and math.isfinite(value) and abs(value) <= np.finfo(np.float32).max:
            values[index] = value
            mask[index] = 1.0
...
def prepare_universal_dataset(config: UniversalPreparationConfig) -> dict[str, Any]:
    ...
    for record in records:
        if sha256_file(Path(record["sourcePath"])) != record["sourceSHA256"]:
            raise ReverseKey1Error("native source identity differs from the prepared manifest")
    command = [
        config.exiftool,
        "-json",
        "-b",
    ...
    if not isinstance(inventory, list) or len(inventory) != len(records):
        raise ReverseKey1Error("universal inventory must contain exactly one record per native source")
    ...
    for index, record in enumerate(records):
        if sha256_file(Path(record["sourcePath"])) != record["sourceSHA256"]:
            raise ReverseKey1Error("native source changed during label extraction")
        sample_path = (source_root / str(record["samplePath"])).resolve()
        sample_sha = sha256_file(sample_path)
        _sample_arrays(sample_path, sample_sha, record["sourceSHA256"])
...
def _verified_archive(path: Path, expected_sha: str) -> dict[str, np.ndarray]:
    payload = path.read_bytes()
    if re.fullmatch(r"[0-9a-f]{64}", expected_sha) is None or hashlib.sha256(payload).hexdigest() != expected_sha:
        raise ReverseKey1Error(f"research archive identity mismatch: {path}")
    with np.load(io.BytesIO(payload), allow_pickle=False) as archive:
        return {name: np.asarray(archive[name]) for name in archive.files}


def _sample_arrays(path: Path, expected_sha: str, source_sha: str) -> dict[str, np.ndarray]:
    arrays = _verified_archive(path, expected_sha)
    if (arrays.get("schema", np.array([])).shape != (1,)
            or str(arrays["schema"][0]) != SAMPLE_SCHEMA
            or arrays.get("source_sha256", np.array([])).shape != (1,)
            or str(arrays["source_sha256"][0]) != source_sha):
        raise ReverseKey1Error("cached sample has mismatched source identity or schema")
    images, key1, mask = (arrays.get(name, np.array([])) for name in ("images", "key1", "mask"))
    if images.shape != (2, 3, INPUT_SIZE, INPUT_SIZE) or images.dtype != np.uint8:
        raise ReverseKey1Error("cached RGB image pair has invalid shape or dtype")
    if key1.shape != (12, 12, 8, 10, 3) or key1.dtype.kind != "f" or not np.isfinite(key1).all():
        raise ReverseKey1Error("cached key1 labels have invalid shape or non-finite values")
    if mask.shape != (12, 12) or mask.dtype != np.bool_ or not mask.any():
        raise ReverseKey1Error("cached key1 mask must contain observed lattice cells")
    return arrays


def _universal_labels(path: Path, header: Mapping[str, Any]) -> dict[str, np.ndarray]:
    count = header["sampleCount"]
    arrays = _verified_archive(path.parent / "labels.npz", str(header.get("labelsSHA256", "")))
    expected = {"gtc": (count, 516), "light_maps": (count, 2, 32, 32),
                "scalars": (count, len(STYLE_SCALAR_FIELDS)),
                "scalar_mask": (count, len(STYLE_SCALAR_FIELDS)),
                "metadata": (count, len(METADATA_FIELDS)),
                "metadata_mask": (count, len(METADATA_FIELDS))}
    if set(arrays) != set(expected):
        raise ReverseKey1Error("universal label fields differ from the declared contract")
    for name, shape in expected.items():
        array = arrays[name]
        if array.shape != shape or array.dtype.kind not in "buif" or not np.isfinite(array).all():
            raise ReverseKey1Error(f"invalid universal label shape or values: {name}")
        if name.endswith("_mask") and not np.isin(array, (0, 1)).all():
            raise ReverseKey1Error(f"universal observation mask must be binary: {name}")
    if arrays["gtc"].dtype != np.uint8:
        raise ReverseKey1Error("GTC labels must preserve the original native uint8 bytes")
    return arrays


def load_universal_manifest(path: Path) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    value = json.loads(path.read_text(encoding="utf-8"))
    header, records = value.get("header"), value.get("samples")
    if not isinstance(header, dict) or header.get("schema") != DATASET_SCHEMA:
        raise ReverseKey1Error("invalid universal manifest; regenerate v2 identities from verified native inputs")
    if not isinstance(records, list) or not records:
        raise ReverseKey1Error("universal records must be a non-empty array")
    actual = hashlib.sha256(canonical_json_bytes(records)).hexdigest()
    if actual != header.get("recordsSHA256") or header.get("sampleCount") != len(records):
        raise ReverseKey1Error("universal record identity or count mismatch")
    sessions: dict[str, set[str]] = defaultdict(set)
    sources: set[str] = set()
    for index, record in enumerate(records):
        if not isinstance(record, dict):
            raise ReverseKey1Error("universal record must be an object")
        if type(record.get("index")) is not int or record["index"] != index:
            raise ReverseKey1Error("universal label indices must be unique, ordered and contiguous")
        split, session = record.get("split"), record.get("captureSession")
        source = record.get("sourceSHA256")
        if split not in ("train", "calibration", "heldout") or not isinstance(session, str) or not session:
            raise ReverseKey1Error("universal split and capture session must be explicit")
        if not isinstance(source, str) or re.fullmatch(r"[0-9a-f]{64}", source) is None or source in sources:
            raise ReverseKey1Error("universal source identities must be unique lowercase SHA-256")
        sources.add(source)
        sessions[split].add(session)
        _sample_arrays(Path(record["samplePath"]), str(record.get("sampleSHA256", "")), source)
...
def universal_state_statistics(
    ...
    header, verified_records = load_universal_manifest(manifest)
    if list(records) != verified_records:
        raise ReverseKey1Error("statistics records must match the verified manifest")
    labels = _universal_labels(manifest, header)
...
def _warm_start_model_state(
    ...
    *,
    ...
    expected = set(migrated) - _STATISTIC_STATE_KEYS
    actual = set(checkpoint_state) - _STATISTIC_STATE_KEYS
    if expected != actual:
        raise ReverseKey1Error(
            f"warm-start learned parameters differ: missing={sorted(expected - actual)}, "
...
class _UniversalDataset:
    def __init__(
        ...
        header, verified_records = load_universal_manifest(manifest)
        if any(record not in verified_records for record in self.records):
            raise ReverseKey1Error("dataset records must be a subset of the verified manifest")
        self.labels = _universal_labels(manifest, header)
    ...
    def __getitem__(self, item: int) -> tuple[Any, ...]:
        ...
        sample = _sample_arrays(Path(record["samplePath"]), record["sampleSHA256"], record["sourceSHA256"])
        images = sample["images"]
        key1 = sample["key1"].astype(np.float32)
        key_mask = sample["mask"]
        ...
        if record.get("linearRGBPath"):
            linear_rgb = linear_sidecar_features(Path(str(record["linearRGBPath"])), expected_sha=record["linearRGBSHA256"])
            modality_mask[0] = 1.0
        if record.get("gainMapPath"):
            gain_map = gain_map_sidecar_features(Path(str(record["gainMapPath"])), expected_sha=record["gainMapSHA256"])
            modality_mask[1] = 1.0
...
def _losses(
    ...
    total = sum(
        torch.exp(-task_noise[i]) * loss + task_noise[i]
        for i, loss in enumerate(values) if i != 3 or scalar_observed
    )
...
def _evaluate(
    ...
    *,
    ...
    consumer_proxy_squared_errors: list[float] = []
    ...
    with torch.no_grad():
        for original in loader:
            ...
            consumer_proxy_squared_errors.extend(
                consumer_delta.square().mean(dim=(1, 2, 3)).cpu().tolist()
            )
            ...
            for index in range(len(normalized_scalars)):
                selected_scalars = normalized_scalars[index][batch[8][index]]
                if selected_scalars.numel():
                    scalar_errors.append(float(selected_scalars.mean().cpu()))
                    scalar_observations += selected_scalars.numel()
    ...
    return {
        ...
        "scalarsNormalizedMAE": float(np.mean(scalar_errors)) if scalar_errors else None,
        "scalarObservations": scalar_observations,
        "unstyledMAE": float(np.mean(unstyled_errors)),
        "consumerProxyMAE": float(np.mean(consumer_proxy_errors)),
        "consumerProxyRMSE8": float(
            np.sqrt(np.mean(consumer_proxy_squared_errors)) * 255.0
        ),
...
def train_universal_model(config: UniversalTrainingConfig) -> dict[str, Any]:
    ...
    if not math.isfinite(config.consumer_weight) or config.consumer_weight < 0:
        raise ReverseKey1Error("consumer weight must be finite and non-negative")
    if (not isinstance(config.epochs, int) or isinstance(config.epochs, bool) or config.epochs <= 0
            or not isinstance(config.batch_size, int) or isinstance(config.batch_size, bool)
            or config.batch_size <= 0):
        raise ReverseKey1Error("epochs and batch size must be positive integers")
    if not math.isfinite(config.learning_rate) or config.learning_rate <= 0:
        raise ReverseKey1Error("learning rate must be finite and positive")
    ...
    if checkpoint_path is not None:
        checkpoint = torch.load(
            checkpoint_path.resolve(), map_location=device, weights_only=True
        )
        if (
    ...
    report = {
        ...
        "claimBoundary": (
            "Offline metrics for the supplied hash-locked corpus only. Producer provenance "
            "is inherited from the source manifest, not proven by this trainer. "
            "Arbitrary-device quality, native consumer compatibility and Photos editing "
