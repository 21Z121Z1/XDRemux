...
The Apple portrait bridge requires one coherent OPPO resource bundle containing
an ISO/TS 21496-1 Gain Map, `rear.depth`, `rear.depth.config`, and a complete
`src.image`. Regardless of whether the outer container is HEIC or JPEG, XDRemux
passes only the unblurred Base JPEG and paired Gain Map JPEG inside `src.image`
to ImageIO and converts it with `kCGImageDestinationPreserveGainMap`. ImageIO-
readable RGB `444f` and grayscale `L008` sources retain their channel structure;
missing, damaged, or 4:2:0 source Gain Maps fail closed. Use `--glob '*.jpg'`
to select JPEG portraits in batch mode. JPEG input is accepted only when
`--apple-portrait` is enabled; Photographic Styles may be enabled alongside it,
while default ISO, Styles-only, and OPPO-compatible modes retain their existing
HEIC-only input contract.

Both Apple switches can be enabled together. XDRemux still creates one HEIC
that keeps HDR, Photographic Styles, and portrait editing. The Portrait writer
and Styles payload share one Vision semantic analysis, which produces person,
skin, hair, facial-hair, teeth, glasses, and sky masks as one request batch
instead of repeating inference for the second Apple feature. During batch
...
| Path | Purpose |
| --- | --- |
| `Package.swift` | Root SwiftPM manifest for two libraries and the `xdremux` executable. |
...
| `Sources/XDRemuxCLI/` | Command dispatch, shared argument parsing, and terminal output. |
