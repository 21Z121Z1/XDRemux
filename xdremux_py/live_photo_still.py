...
def read_apple_content_identifier(path: Path) -> str | None:
    ...
    try:
        with Image.open(path) as image:
            ...
            if maker is None:
                # Compatibility for files generated by pre-fix XDRemux builds; new
                # output is always written into the standards-defined Exif IFD.
                maker = exif.get(APPLE_MAKERNOTE_TAG)
    except Exception:
        return None
