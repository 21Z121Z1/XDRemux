...
def read_apple_content_identifier(path: Path) -> str | None:
    """Read only a standards-placed Apple MakerNote from the Exif IFD.

    A historical development build wrote tag 0x927C into IFD0. Apple ImageIO
    does not recognize that placement as a MakerNote, so accepting it here would
    make skip-existing/pair validation disagree with the Apple compatibility
    oracle. Deliberately reject that malformed layout.
    """
    import pillow_heif
